        ...
        io_uring_for_each_cqe(&ring, head, cqe) {
            ...
            if (cqe->res == -ENOBUFS) {
                ...
                if (cqe->res <= 0) {
                    ...
                    add_socket_write(&ring, conn_i.fd, 0, bytes_read, 0);
                }
            } else if (type == WRITE) {
                ...
                add_socket_read(&ring, conn_i.fd, 0, MAX_MESSAGE_LEN, 0);
            }
        }
...
void add_socket_read(struct io_uring *ring, int fd, unsigned gid, size_t message_size, unsigned flags) {
	struct io_uring_sqe *sqe = io_uring_get_sqe(ring);
    io_uring_prep_recv(sqe, fd, bufs[fd], message_size, 0);
...
void add_socket_write(struct io_uring *ring, int fd, __u16 bid, size_t message_size, unsigned flags) {
    struct io_uring_sqe *sqe = io_uring_get_sqe(ring);
    io_uring_prep_send(sqe, fd, bufs[fd], message_size, 0);
    io_uring_sqe_set_flags(sqe, 0);
