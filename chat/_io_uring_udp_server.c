...
static void broadcast_to_all(struct ctx *ctx, int fdidx, const void *data, size_t len) {
	for (size_t i = 0; i < ctx->client_count; i++) {
		...
		io_uring_sqe_set_data64(sqe, BUFFERS + i);
...
static int setup_context(struct ctx *ctx)
{
	...
	params.flags = IORING_SETUP_SUBMIT_ALL | IORING_SETUP_CQSIZE | IORING_SETUP_SQPOLL;
	// params.flags = IORING_SETUP_SUBMIT_ALL | IORING_SETUP_CQSIZE | IORING_SETUP_SQPOLL | IORING_SETUP_COOP_TASKRUN;

	params.sq_thread_idle = 9999;
			    ...
			    int fdidx)
{
	...
	add_client(ctx, ctx->send[idx].msg.msg_name, ctx->send[idx].msg.msg_namelen);
	broadcast_to_all(ctx, fdidx, ctx->send[idx].iov.iov_base, ctx->send[idx].iov.iov_len);
